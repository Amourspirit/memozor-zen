...
// @downloadURL     https://github.com/Amourspirit/memozor-zen/raw/master/dist/zen-www-memozor-com.user.js
...
(function($) {
		...
		var DispatcherBase = (function() {
			...
			DispatcherBase.prototype._dispatch = function(executeAsync, scope, args) {
				...
				var _loop_1 = function(sub) {
					var ev = new management.EventManagement(function() {
			...
			if (this.lEvents.hasOwnProperty(key)) {
				Log.Error(
					appSettings.shortName +
						": addElement: key " +
						key +
						" is already in the list of elemets and can not be added again",
				);
				return;
			}
			this.lEvents[key] = e;
			...
			if (this.lEvents.hasOwnProperty(args.key) === false) {
				Log.Error(
					appSettings.shortName +
						": elementLoaded: key " +
						args.key +
						" was not found to delete. This may be a serious error",
				);
				return;
			} else {
	...
	var lib = createCommonjsModule(function(module, exports) {
		...
		var ElLocation;
		(function(ElLocation) {
			ElLocation[(ElLocation["head"] = 0)] = "head";
			ElLocation[(ElLocation["body"] = 1)] = "body";
			ElLocation[(ElLocation["other"] = 2)] = "other";
		})((ElLocation = exports.ElLocation || (exports.ElLocation = {})));
		exports.elementsCreate = function(eArgs) {
			var parentEl = elementCreate(eArgs);
			if (eArgs.children) {
				addElementRecursive(parentEl, eArgs.children);
			}
			return parentEl;
		};
		var elementCreate = function(eArgs) {
			var htmlNode = genericCreateElement(eArgs.tag);
			if (eArgs.attribs) {
		...
		var genericCreateElement = function(tag) {
			var D = document;
			var node = D.createElement(tag);
			return node;
		};
		var addElementRecursive = function(parentElement, eArgs) {
	...
	unwrapExports(lib);
	var lib_1 = lib.ElLocation;
	var lib_2 = lib.elementsCreate;
	var lib_3 = lib.elementAddToDoc;
	var lib_4 = lib.elementAddNewToDoc;
	var ElementLoad = (function(_super) {
		...
		ElementLoad.prototype.onTickTock = function(eventArgs) {
			...
			if (this.lArgs.elementCreate.children) {
				var multiHtml = lib_2(this.lArgs.elementCreate);
				lib_3(multiHtml, this.lArgs.scriptLocation);
			} else {
				var eHtml = lib_2(this.lArgs.elementCreate);
				lib_3(eHtml, this.lArgs.scriptLocation);
			}
	...
	var MainElementLoader = (function(_super) {
		...
		MainElementLoader.prototype.addStyleCss = function() {
			this.addStyle("styleCss", this.getStyleCss(), lib_1.head);
		};
		...
		MainElementLoader.prototype.addStyle = function(key, styelcontent, elementLocation) {
			if (elementLocation === void 0) {
				elementLocation = lib_1.head;
			}
	...
	var Fullscreen = (function() {
		...
		Fullscreen.prototype.init = function() {
			...
			var rx = /https?:\/\/(?:www\.)?memozor\.com\/memory-game-online-free\/.*/;
			if (rx.test(href)) {
		...
		Fullscreen.prototype.injectButton = function() {
			...
			if (!divBtnHolder.length) {
				Log.Error(
					appSettings.shortName +
						" could not find where to place button: selector: " +
						appSettings.buttonPlacementSelector,
				);
				return;
			}
		...
		Fullscreen.prototype.getGameWrapper = function() {
			...
			return lib_2(htmlArgs);
		};
		Fullscreen.prototype.getWrapperBgStyle = function() {
			var gb = $(appSettings.gameBoardSelector);
			var c = gb.css("background-color");
		...
		iv_1.onTick().subscribe(function(s, a) {
			if ($(appSettings.gameBoardSelector).length === 1) {
				...
				loader_1.onAllElementsLoaded().subscribe(function(sender, args) {
					loader_1.dispose();
					Log.Info(
						appSettings.shortName + ": Entry Script: All Scripts loaded. Total count: " + args.totalNumberOfScripts,
					);
					main();
				});
				loader_1.onElementsLoadFail().subscribe(function(sender, args) {
					loader_1.dispose();
					Log.Error(
						appSettings.shortName + ": Entry Script: The neceassary elements were note loaded. Failed:",
						args.remainingEvents,
					);
				});
				...
				loader_1.onTickExpired().subscribe(function(sender, args) {
					Log.Warn(
						appSettings.shortName + ": Entry Script: Element with Key value of '" + args.key + "' has failed to load",
					);
				});
				loader_1.start();
			}
